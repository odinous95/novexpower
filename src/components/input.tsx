...
export function Input({ id, name, label, type, disabled, error, value, onChange }: Props) {
  return (
    <div className="relative w-full">
      ...
      <label
        htmlFor={id}
        className="pointer-events-none absolute left-4 top-3 z-20 origin-[0] -translate-y-4 scale-75 text-xs text-slate-500 transition duration-150 peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-focus:-translate-y-4 peer-focus:scale-75 peer-focus:text-emerald-600 dark:text-slate-300 dark:peer-focus:text-emerald-300 sm:left-5 sm:top-4"
      >
        {label}
      </label>
      {error && (
